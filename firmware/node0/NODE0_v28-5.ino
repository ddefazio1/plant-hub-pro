...
const char* MQTT_PASS     = "mqtt11";     // <-- PUT YOUR PASSWORD HERE
...
/* ================= REBOOT TRACKING ================= */
RTC_NOINIT_ATTR int bootCount;  // Survives soft reboots, reset on power loss
RTC_NOINIT_ATTR unsigned long lastUptimeBeforeReboot;
...
void logReboot() {
  ...
  File f = SD.open("/reboot_log.csv", FILE_APPEND);
  if (!f) {
    // Create with header
    f = SD.open("/reboot_log.csv", FILE_WRITE);
    if (!f) return;
    f.println("timestamp,boot_count,reason,prev_uptime_min,heap");
  }
  f.printf("%s,%d,%s,%lu,%d\n",
           ts, bootCount, getResetReason().c_str(),
           lastUptimeBeforeReboot / 60000,
           ESP.getFreeHeap());
  f.close();
  Serial.printf("[REBOOT LOG] #%d - Reason: %s - Prev uptime: %lu min\n",
                bootCount, getResetReason().c_str(), lastUptimeBeforeReboot / 60000);
}
...
String getNOAAGridPoint() {
  // NOTE: Watchdog already disabled by caller (updateWeather)
  HTTPClient http;
  ...
  if (httpCode == 200) {
    ...
    Serial.printf("[WEATHER] Grid point failed: %d\n", httpCode);
  }
  http.end();
  return forecastURL;
}
...
void updateWeather() {
  esp_task_wdt_delete(NULL);  // Disable watchdog - SSL can block 30+ sec
  ...
  if (httpCode == 200) {
    ...
    Serial.printf("[WEATHER] Forecast failed: %d\n", httpCode);
    weather.valid = false;
  }
  http.end();
  esp_task_wdt_add(NULL);  // Re-enable watchdog
...
void mqttSendAllDiscovery() {
  ...
  mqttDiscovery("boot_count", "Plant Hub Boot Count", "planthub/system", "", "", "mdi:restart", "{{ value_json.boots }}");
  mqttDiscovery("reset_reason", "Plant Hub Reset Reason", "planthub/system", "", "", "mdi:alert-circle", "{{ value_json.reset }}");
...
void mqttPublish() {
  ...
  // System
  snprintf(buf, sizeof(buf),
           "{\"uptime\":%lu,\"heap\":%d,\"battery\":%.2f,\"nodes\":%d,\"boots\":%d,\"reset\":\"%s\",\"ip\":\"%s\"}",
...
void serveJSON(WiFiClient& client) {
  ...
  client.print("\"bootCount\":"); client.print(bootCount); client.print(",");
  client.print("\"resetReason\":\""); client.print(getResetReason()); client.print("\",");
...
void setup() {
  ...
  Serial.printf("Boot #%d | Reset reason: %s\n", bootCount, getResetReason().c_str());
  Serial.printf("Previous uptime: %lu min\n", lastUptimeBeforeReboot / 60000);
  Serial.println("========================================\n");
  if (esp_reset_reason() == ESP_RST_POWERON) bootCount = 0;
  bootCount++;
...
void loop() {
  ...
  if (millis() - lastBeacon > 60000) {
    broadcastBeacon();
    lastUptimeBeforeReboot = millis();  // Save uptime in RTC memory
    lastBeacon = millis();
  }
